...
from mini_wins.dialog_win import MiniWindow
...
class Scene1(QWidget):
    ...
    def Get_months(self):
        lst = []
        with open("txt_files\months_2.txt" , "r", encoding="utf-8") as plik:
    ...
    def Calendar_lay(self,arg_date):
        self.weeks = ["Poniedziałek", "Wtorek", "Środa", "Czwartek", "Piątek", "Sobota", "Niedziela"]
        ...
        Mon_label = QLabel(self.weeks[current_date.dayOfWeek()-1].lower() + ", " + str(current_date.day()) + 
                           " " + name + " " +  str(current_date.year()),Mon_frame)
        ...
        for row in range(2,8):
            for column in range(7):
                ...
                if date.month() != current_date.month():
                    frame.setStyleSheet("background-color: #EBEBEB;"
                                           "color: lightgrey;")
                else:
